"""The run log: a live record of what a pipeline is doing.
...
Records go in the configured cache directory, under ``runs/``.  That is the
...
The run log is a diagnostic, never a dependency.  Every failure here is
...
rather than filling the disk.  Old run files are pruned when a new one is
opened.
"""
...
def set_role(role: str) -> None:
    ...
    be called before the first record, since the role is recorded in the run
    header.
    """
...
def record(store: Any, ev: str, **fields: Any) -> None:
    """Write one record to *store*'s run log.
    ...
    try:
        ...
        if _writer is None or root != _writer_root:
            if _writer is not None:
                _writer.close()
            _writer = _Writer(Path(root) / "runs")
            _writer_root = root
